...
import play.api.libs.json.{Format, JsBoolean, JsError, JsNull, JsObject, JsResult, JsString, JsSuccess, JsValue, Json, OWrites, Reads, Writes, __}
...
class EmailSetSerializer @Inject()(messageIdFactory: MessageId.Factory) {
  ...
  private implicit val updatesMapReads: Reads[Map[UnparsedMessageId, EmailSetUpdate]] =
    readMapEntry[UnparsedMessageId, EmailSetUpdate](s => refineV[UnparsedMessageIdConstraint](s),
      o => emailSetUpdateReads.reads(o))
