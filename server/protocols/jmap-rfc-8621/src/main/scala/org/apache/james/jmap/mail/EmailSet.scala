  ...
  private def createMultipartWithAttachments(maybeHtmlBody: Option[String],
    ...
    maybeAttachments.map(list => {
      (list.filter(_._1.isInline), list.filter(!_._1.isInline)) match {
        case (inlineAttachments, normalAttachments) if inlineAttachments.isEmpty => createMixedBody(maybeHtmlBody, normalAttachments, htmlTextExtractor)
        case (inlineAttachments, normalAttachments) if normalAttachments.isEmpty => createRelatedBody(maybeHtmlBody, inlineAttachments, htmlTextExtractor)
